...
先在行情页顶部「证券目录」勾选指数、A股、ETF并点击「同步证券目录」。后台从QMT读取支持范围内的完整目录与真实名称，显示进度，支持停止、任务页重试和服务重启恢复。重复点击会返回当前活动任务；只同步资料，不下载全部证券的历史行情，也不自动删除旧证券。

行情证券、历史查询和数据集成员均通过选择器按代码或名称搜索；行情和数据集使用复选，历史和指数使用单选。可筛选类别、分页、勾选本页、仅看已选与清空选择，跨页保留选项。已同步目录可以在QMT离线时搜索。
...
catalog_job = client.sync_catalog()  # 后台同步指数、A股、ETF资料
