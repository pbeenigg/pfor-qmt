...
def test_preview_cancel_repair_links_and_reverify(store,tmp_path,monkeypatch):
    ...
    try:
        with sync_playwright() as pw:
            ...
            page.locator('[data-task-tab=links]').click()
            expect(page.locator('#task-body')).to_contain_text('核验前任务')
            page.locator('[data-task-tab=summary]').click()
            ...
            page.locator(f'#job-rows [data-job-detail="{verification["id"]}"]').click()
            page.locator('[data-task-tab=links]').click()
            expect(page.locator('#task-body')).to_contain_text('缺口补数')
            page.locator(f'#task-body [data-job-detail="{created["id"]}"]').click()
            page.locator('[data-task-tab=links]').click()
            expect(page.locator('#task-body')).to_contain_text('补数依据')
            page.locator(f'#task-body [data-job-detail="{verification["id"]}"]').click()
            ...
            expect(page.locator('#task-body')).to_contain_text('校验通过')
