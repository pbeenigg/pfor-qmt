...
本轮完整结果为`102 passed in 15.39s`，0失败、0跳过。初轮暴露Path类型运行目录被严格校验拒绝，已修复并新增回归，未跳过失败测试。

用户指定的本机PostgreSQL16.15已连接。检查发现pfor_qmt数据库尚不存在，已创建指定数据库并迁移同名schema，版本1，共10张表；行情表0行。未向业务库写入测试数据，自动化测试仍在独立PostgreSQL14容器中运行。

工作台现从config.toml读取真实连接。数据库凭据未写入源码、日志、测试或发布包；原settings.local.json仅作为本地备份保留。QMT目录仍未配置，真实行情仍未验收。
...
用户PostgreSQL连接和迁移已在上述统一配置更新中验证。尚未提供QMT根目录，没有修改真实QMT模型或触发行情下载，也没有账户交易行为。
