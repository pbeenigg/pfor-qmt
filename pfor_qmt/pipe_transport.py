...
DEFAULT_PIPE_NAME = os.environ.get("PFOR_QMT_PIPE_NAME", r"\\.\pipe\pfor_qmt_pipe_hub")
...
class PipeTxClient(object):
    ...
    def __init__(
        ...
        if heartbeat_interval is None:
            heartbeat_interval = os.environ.get("PFOR_QMT_PIPE_HEARTBEAT_SECONDS", "10")
        try:
            self.heartbeat_interval = max(0.0, float(heartbeat_interval))
        except Exception:
            self.heartbeat_interval = 10.0
        self.process_id = os.getpid()
        self.instance_id = os.environ.get("PFOR_QMT_PIPE_INSTANCE_ID") or "%s.%s.%s.%s" % (
            self.bridge_id,
            self.endpoint_name,
            self.process_id,
            uuid.uuid4().hex[:12],
