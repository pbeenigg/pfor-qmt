...
_TRANSLATIONS = [
    ...
    (
        ...
        "行情普通桥 ContextInfo 已就绪 版本={version}",
    ),
    (
        r"^cfquant normal bridge timer cancel failed:(?P<error>.+)$",
        "行情普通桥定时器取消失败：{error}",
    ),
    (r"^cfquant normal bridge stopped$", "行情普通桥已停止"),
    (
        r"^cfquant callback publish failed event=(?P<event>[^ ]+) error=(?P<error>.+)$",
        "行情回调事件发布失败 event={event} 错误={error}",
    ),
    (r"^cfquant lowlat trade bridge module loaded$", "极速交易桥模块已加载"),
    (r"^cfquant lowlat entry version:(?P<version>.+)$", "极速交易入口版本：{version}"),
    (
        r"^cfquant bridge id:(?P<bridge_id>[^ ]+) trade_channel:(?P<trade>.+)$",
        "行情桥接ID={bridge_id} 交易通道={trade}",
    ),
    (
        r"^cfquant lowlat trade context ready version:(?P<version>.+)$",
        "极速交易桥 ContextInfo 已就绪 版本={version}",
    ),
    (r"^cfquant lowlat trade bridge stopped$", "极速交易桥已停止"),
    (r"^cfquant ctypes all-in-one lowlat bridge module loaded$", "ctypes 单文件低延迟桥模块已加载"),
    (r"^cfquant ctypes all-in-one lowlat entry version:(?P<version>.+)$", "ctypes 单文件低延迟入口版本：{version}"),
    (
        ...
        "ctypes 桥接ID={bridge_id} pipe={pipe} 普通通道={normal} 交易通道={trade} 回调通道={callback}",
    ),
    (
        r"^cfquant ctypes trade loop in thread:(?P<thread>[^ ]+) sleep_seconds:(?P<sleep>.+)$",
        "ctypes 交易循环后台线程={thread} 轮询间隔={sleep}秒",
    ),
    (
        r"^cfquant ctypes lowlat trade loop error:(?P<error>.+)$",
        "ctypes 低延迟交易循环异常：{error}",
    ),
    (r"^cfquant ctypes lowlat trade loop started in worker thread$", "ctypes 低延迟交易循环已在后台线程启动"),
    (r"^cfquant ctypes lowlat trade loop entering current QMT thread$", "ctypes 低延迟交易循环进入当前 QMT 线程"),
    (
        r"^cfquant ctypes lowlat trade dispatch error source=(?P<source>[^ ]+) error=(?P<error>.+)$",
        "ctypes 低延迟交易请求派发异常 来源={source} 错误={error}",
    ),
    (
        r"^cfquant ctypes lowlat trade timer scheduled key:(?P<key>[^ ]+) interval_ms:(?P<interval>.+)$",
        "ctypes 低延迟交易定时器已注册 key={key} 间隔={interval}ms",
    ),
    (
        r"^cfquant ctypes lowlat trade timer schedule failed:(?P<error>.+)$",
        "ctypes 低延迟交易定时器注册失败：{error}",
    ),
    (
        r"^cfquant ctypes lowlat trade timer cancel failed:(?P<error>.+)$",
        "ctypes 低延迟交易定时器取消失败：{error}",
    ),
    (
        r"^cfquant ctypes normal context ready version:(?P<version>.+)$",
        "ctypes 普通桥 ContextInfo 已就绪 版本={version}",
    ),
    (
        r"^cfquant ctypes lowlat trade context ready version:(?P<version>.+)$",
        "ctypes 低延迟交易桥 ContextInfo 已就绪 版本={version}",
    ),
    (r"^cfquant ctypes lowlat trade bridge stopped$", "ctypes 低延迟交易桥已停止"),
    (r"^cfquant ctypes normal bridge stopped$", "ctypes 普通桥已停止"),
    (
        r"^cfquant ctypes lowlat callback publish failed event=(?P<event>[^ ]+) error=(?P<error>.+)$",
        "ctypes 回调事件发布失败 event={event} 错误={error}",
    ),
    (
